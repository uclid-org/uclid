...
\paratitle{Instance declarations} allow the instantiation (duh!) of other modules. It consists of the instance name, the name of the module being instantiated and the list of mappings for the instances' inputs, output and shared variables. 
\begin{grammar}
...
\paratitle{Axioms} refer to assumptions while a \paratitle{specification declaration} can be a \keywordbf{property} or \keywordbf{invariant} of the design. Note \keywordbf{property} and \keywordbf{invariant} are synonyms.

\begin{grammar}
     <AxiomDecl> ::= \keywordbf{axiom} <Id> `:' <Expr> `;'
                 \alt \keywordbf{axiom} <Expr> `;'

     <SpecDecl> ::= <PropertyKW> <Id> `:' <Expr> `;'
                 \alt <PropertyKW> <Expr> `;'

     <PropertyKW> ::= \keywordbf{property} \alt \keywordbf{invariant}
\end{grammar}
