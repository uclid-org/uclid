...
\paragraph{Building from source (optional)} The supplied \artcommand{sbt} Debian package is only required if you wish to build \uclidcmd from source. Start by installing \artcommand{sbt} by running \artcommand{sudo dpkg -i sbt\_1.5.5\_all.deb} from the \artcommand{packages} directory. From the \artcommand{uclid} directory, follow the instructions given at \url{https://github.com/uclid-org/uclid#compiling-uclid5}. After doing so, you can either build the \uclidcmd binary or run examples directly from within the \artcommand{}{sbt} prompt.
...
\begin{itemize}
    \item Prebuilt binaries for the \uclidcmd tool and the external solvers \artcommand{z3}, \artcommand{cvc4} and \artcommand{delphi} in their respective directories
    \item Five test-examples directories: \artcommand{Fib}, \artcommand{Control},
    \artcommand{TrainSystem},
    \artcommand{OperAxUhb}, and
    \artcommand{TrustedAbstractPlatform}
    \item Run scripts for each of the above examples, named as \artcommand{run\_<name>.sh}
    \item A \artcommand{export\_paths} sourcefile
\end{itemize}


\paragraph{Startup instructions}
To begin, source the \artcommand{export\_paths} script. 
This should set permissions for running all the binaries and add required paths to the \artcommand{\$PATH} variable.
\begin{equation*}
    \artcommand{source export\_paths}
\end{equation*}
To run example \artcommand{examplename}, 
run the script \artcommand{./run\_<examplename>.sh} 
from the \artcommand{tool\_paper\_examples} directory.
Here \artcommand{examplename} corresponds to:
\artcommand{Fib}, \artcommand{Control},
\artcommand{TrainSystem}, \artcommand{OperAxUhb}, and \artcommand{TrustedAbstractPlatform}.
...
\subsection{\excommand{TrustedAbstractPlatform}}
\input{example_TrustedAbstractPlatform.tex}
